...
| --- | --- |
| Compute | Five Proxmox hosts: Trinity, Neo, and Niobe run schedulable amd64 Talos control-plane VMs; Smith runs the amd64 Talos GPU worker VM; Oracle hosts core and edge guests such as VyOS, the home gateway, identity, DNS, bastion, and IDS services. Four bare-metal Raspberry Pi ARM workers, `talos-mouse`, `talos-dozer`, `talos-tank`, and `talos-sparks`, complete the same Talos cluster |
