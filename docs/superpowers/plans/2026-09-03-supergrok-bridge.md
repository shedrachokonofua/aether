...
- [ ] **Step 8b: Verify forced tool calls survive translation.** Send one Chat Completions request through LiteLLM with a single declared function and `tool_choice: {"type":"function","function":{"name":...}}`; confirm the response is a `tool_calls` finish for that function, not text. This is the capability the Cursor route cannot provide (Cursor runs its own agent loop and never honours `tool_choice`): Colony's staged architect steers grok-4.6 onto its submit tool after the turn cap, and on `cursor/grok-4.6` the model narrated "submitting" three times and called nothing (qwen-lab `staged2/grok-4.6-c`, 2026-09-03). A forced call landing here is the evidence to re-rank grok as architect; Colony's post-cap steer then sets `tool_choice` on `supergrok/*` routes.
