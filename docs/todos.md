...
- [ ] Prepare repo for open source
  - [x] Add pre-commit hooks for secret detection
    - [x] Create `.pre-commit-config.yaml` with gitleaks and custom SOPS checks
    - [x] Add `.gitleaks.toml` for custom rules
  ...
  - [x] Update SOPS workflow to never decrypt in place
    - [x] Add `sops:edit` task (decrypts to /tmp, re-encrypts on save)
    - [x] Add `sops:view` task (stdout only)
    - [x] Add `sops:get` task (single value extraction)
    - [x] Deprecate `sops:decrypt` / `sops:encrypt` in-place tasks
  - [x] Add `.sops.yaml` config file to document encryption expectations
