...
- Namespace ResourceQuota (`namespace_resourcequota_gitlab_runner` in
  ...
  `namespace-resourcequota` policy): sized for 8 counted pods (4 running + 4
  Terminating). The generating policy only re-syncs on namespace
  CREATE/UPDATE, so quota changes must also bump the
  `aether.shdr.ch/runner-quota-rev` namespace annotation in
