...
- Helper container requests/limits: `100m`/`2` CPU, `128Mi`/`512Mi` memory,
  `512Mi`/`1Gi` ephemeral storage
- Service container requests/limits: `100m`/`500m` CPU, `128Mi`/`512Mi` memory,
  `512Mi`/`1Gi` ephemeral storage
- Pod termination/cleanup grace: `5s` — job containers ignore SIGTERM after the
  build finishes, and the `gitlab-runner` namespace ResourceQuota counts
  Terminating pods, so the default 30s grace caused quota rejections
  (`runner_system_failure`) during pipeline fan-out bursts
- Namespace ResourceQuota (`namespace_resourcequota_gitlab_runner` in
  `tofu/home/kubernetes/kyverno.tf`, generated by the Kyverno
  `namespace-resourcequota` policy): `requests.cpu=18`, `limits.cpu=34`,
  `requests.memory=24Gi`, `limits.memory=40Gi`,
  `requests.ephemeral-storage=96Gi`, `limits.ephemeral-storage=264Gi`, and
  `pods=20`. It is sized for 8 counted job pods (4 running + 4 terminating):
  each contributes `2.1` CPU request and `4` CPU limit (build + helper), while
  four managers contribute `0.4` request and `2.0` limit total. The generating
  policy only re-syncs on namespace CREATE/UPDATE, so quota changes must also
  bump the `aether.shdr.ch/runner-quota-rev` namespace annotation to
  `cpu18-34.mem24-40.eph96-264.r3` in `namespace_contracts.tf`
- Default build storage driver: `STORAGE_DRIVER=overlay`
- Runner cache defaults: `CACHE_COMPRESSION_FORMAT=tarzstd` and
