...
### Talos node exclusions (2026-07-31)

The nightly job (`pbs-backup`, `--all 1`, 02:00) caused a recurring ~06:02 UTC etcd stall:
live vzdump of `talos-neo`'s system disk (Talos EPHEMERAL, where etcd lives) throttled guest
writes via copy-before-write — 108 GiB dirty on 2026-07-31 meant a 47-minute backup, etcd
timeouts, and a multi-node NotReady cascade. Two mitigations, both in IaC:

- **`virtio0` of every Talos VM has `backup=0`** (`tofu/home/talos_cluster.tf`). A node-image
  restore is never the correct recovery for a Talos node; cluster state is covered by the
  daily 02:20 `talosctl etcd snapshot` on `backup-stack` (below). Data disks (neo's GPU
  storage `virtio2`, CI `virtio3`) remain in the backup.
- **`freeze-fs-on-backup=0`** on the Talos VMs' QGA config, enforced by
  `ansible/playbooks/backup_stack/configure_proxmox_backups.yml` (the bpg tofu provider has
  no field for this agent sub-option). Talos gains nothing from filesystem quiesce and etcd
  fsyncs must never be frozen.
...
- Per-namespace backup identities are generated by tofu into
  `tofu/home/secrets/seaweedfs-s3.json` and must be pushed with
  `task seaweedfs:s3-identities:deploy` (runs
  `ansible/playbooks/seaweedfs/deploy_s3_config.yml` and restarts only
  `seaweedfs-s3.service`).
  Applying the k8s secret/ObjectStore alone is not enough — Seaweed will not know the
  new access key until that playbook runs.
- Assay has a separate generated `assay-artifacts` identity restricted to the
  `assay-artifacts/assay/td` application-artifact surface. It does not reuse the
  Seaweed admin identity or the per-namespace database-backup identity.
- Buzz has a separate generated `buzz-media` identity restricted to the
  `buzz-media` bucket (relay media blobs + git object storage). Same pattern:
  no admin identity, no database-backup identity reuse.
...
- Tofu declares CNPG-I Barman Cloud ObjectStore + ScheduledBackup for CNPG clusters (including
  Kestra as of 2026-07-09), destination `s3://aether-db-dumps/cnpg/<ns>/<cluster>`, 14-day retention
- After adding a namespace to `db_backup_namespaces`, regenerate + deploy Seaweed S3 identities
  before expecting WAL archive / base backups to succeed (`task
