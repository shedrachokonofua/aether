-- gigahub-exporter host observations: shared schema v1 / sealed-NDJSON contract.
-- A dedicated MV keeps ingestion routing explicit while writing into the common
-- replay-idempotent network.observations table.
CREATE MATERIALIZED VIEW IF NOT EXISTS network.observations_gigahub_mv
...
FROM network.ingest
WHERE LogAttributes['log.source'] = 'gigahub_observations'
