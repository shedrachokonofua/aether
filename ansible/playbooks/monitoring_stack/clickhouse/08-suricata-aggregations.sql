...
CREATE MATERIALIZED VIEW IF NOT EXISTS suricata.alert_hourly
ENGINE = SummingMergeTree()
ORDER BY (hour, alert_severity, alert_signature)
...
FROM suricata.alert
GROUP BY hour, alert_severity, alert_signature, alert_category;

-- Hourly alert aggregates by source IP (for top attackers)
CREATE MATERIALIZED VIEW IF NOT EXISTS suricata.alert_by_src_hourly
ENGINE = SummingMergeTree()
ORDER BY (hour, src_ip, alert_severity)
...
AS SELECT
    ...
    count() AS alert_count
FROM suricata.alert
GROUP BY hour, src_ip, alert_severity;

-- Hourly flow aggregates by protocol
CREATE MATERIALIZED VIEW IF NOT EXISTS suricata.flow_hourly
ENGINE = SummingMergeTree()
ORDER BY (hour, proto, app_proto)
TTL hour + INTERVAL 90 DAY
AS SELECT
    toStartOfHour(Timestamp) AS hour,
    proto,
