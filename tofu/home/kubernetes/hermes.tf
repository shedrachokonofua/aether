  ...
  hermes_agents = {
    ...
    tungsten = {
      ...
      env = {
        ...
        JELLYFIN_URL           = local.hermes_jellyfin_url
        QBITTORRENT_URL        = "http://${kubernetes_service_v1.qbittorrent.metadata[0].name}.${local.qbittorrent_ns}.svc.cluster.local:${local.qbittorrent_port}"
        SABNZBD_URL            = "http://${kubernetes_service_v1.sabnzbd.metadata[0].name}.${local.media_ns}.svc.cluster.local:${local.sabnzbd_port}"
        SONARR_URL             = "http://${kubernetes_service_v1.sonarr.metadata[0].name}.${local.media_ns}.svc.cluster.local:${local.sonarr_port}"
        RADARR_URL             = "http://${kubernetes_service_v1.radarr.metadata[0].name}.${local.media_ns}.svc.cluster.local:${local.radarr_port}"
        LIDARR_URL             = "http://${kubernetes_service_v1.lidarr.metadata[0].name}.${local.media_ns}.svc.cluster.local:${local.lidarr_port}"
        PROWLARR_URL           = "http://${kubernetes_service_v1.prowlarr.metadata[0].name}.${local.media_ns}.svc.cluster.local:${local.prowlarr_port}"
        MATRIX_HOMESERVER      = local.hermes_matrix_server
