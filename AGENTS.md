...
- `task login` gets AWS, Google WIF, OpenBao, Ceph RGW, OCI, and an SSH certificate in one shot. Credentials are cached; only re-run when `task login:status` or a command shows missing or expired access. (OCI: Keycloak->UPST token-exchange into profile `oci-aether`; the UPST lasts ~1h. Interactive dev shells alias `oci` to `scripts/aether-oci`, a wrapper that silently re-mints before each call - but the alias does NOT apply to non-interactive use: with `nix develop --command`, bare `oci` is the real CLI and can 401 on a stale token, so invoke the wrapper by path (`nix develop --command ./scripts/aether-oci <args>`) or run `task login:oci-renew` first.)
- `task login` starts the SSH cert in an agent at `~/.aether-toolbox/ssh/agent.sock` (note the `ssh/` — **not** `~/.aether-toolbox/agent.sock`, which is a stale/wrong path). A non-interactive shell won't have it wired, so `ssh`/`ssh-add -l` fail with "Error connecting to agent" or "Permission denied". Export it first: `export SSH_AUTH_SOCK=$HOME/.aether-toolbox/ssh/agent.sock`, then `ssh root@<node>` (e.g. the Proxmox hosts) presents the cert and connects.
