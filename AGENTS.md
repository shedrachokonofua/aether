...
- `task login` gets AWS, Google WIF, OpenBao, Ceph RGW, OCI, and an SSH certificate in one shot. Credentials are cached; only re-run when `task login:status` or a command shows missing or expired access. (OCI: Keycloak->UPST token-exchange into profile `oci-aether`; the UPST lasts ~1h — run `task login:oci-renew` (silent, no browser) when OCI calls 401, or just use `./scripts/aether-oci <args>`, an `oci` wrapper that lazily renews before each call, GCP-ADC-style.)
